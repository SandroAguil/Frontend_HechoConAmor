  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <button onClick={() => setMostrarFormulario(true)} className="flex items-center gap-2 px-4 py-2 bg-pastelLavender text-gray-800 rounded-lg shadow hover:bg-pastelBlue transition">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <input type="text" placeholder="Buscar por cliente o código" value={busqueda} onChange={(e) => setBusqueda(e.target.value)} className="w-full sm:w-1/2 p-2 border rounded-lg shadow" />
              ...
              <tr key={i} className="border-b hover:bg-pastelMint transition">
                ...
                <td className="px-4 py-3">
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                    ...
                    'bg-red-100 text-red-700'}`}>{pedido.estado}</span>
                </td>
                <td className="px-4 py-3 text-center flex justify-center gap-3">
                  <button className="text-blue-500 hover:text-blue-700" onClick={() => setPedidoEditandoEstado(pedido)}>✏️</button>
                  <button className="text-red-500 hover:text-red-700" onClick={() => setPedidoEliminando(pedido)}><FaTrash /></button>
                </td>
              </tr>
            ))}
          </tbody>
            ...
            <motion.div className="bg-white p-6 rounded-xl shadow-lg w-full max-w-md" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }}>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex justify-end gap-3">
                  ...
                  <button type="submit" className="px-4 py-2 bg-pastelLavender rounded hover:bg-pastelBlue">Guardar</button>
                </div>
              </form>
            </motion.div>
          </motion.div>
          ...
          <motion.div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <motion.div className="bg-white p-6 rounded-xl shadow-lg w-full max-w-md" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }}>
              ...
              <form onSubmit={(e) => { e.preventDefault(); handleActualizarEstado() }} className="space-y-4">
                ...
                <div className="flex justify-end gap-3">
                  <button type="button" onClick={() => setPedidoEditandoEstado(null)} className="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">Cancelar</button>
          ...
          <motion.div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <motion.div className="bg-white p-6 rounded-xl shadow-lg w-full max-w-sm text-center" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }}>
              ...
              <div className="flex justify-center gap-4">
                <button onClick={() => setPedidoEliminando(null)} className="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">Cancelar</button>
                <button onClick={() => { eliminarPedido(pedidoEliminando.id); setPedidoEliminando(null); toast.success('Pedido eliminado correctamente') }} className="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600">Eliminar</button>
              </div>
            </motion.div>
          </motion.div>
        )}
