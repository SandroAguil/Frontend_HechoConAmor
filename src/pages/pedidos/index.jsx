    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 px-5 py-2 bg-[#FDC5DC] text-gray-800 rounded-xl shadow hover:bg-[#F8A1BF] transition"
        >
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <input
          ...
          className="w-full sm:w-1/2 p-3 border rounded-xl shadow-sm"
        />
        <select
          value={filtro}
          onChange={(e) => setFiltro(e.target.value)}
          className="w-full sm:w-1/4 p-3 border rounded-xl shadow-sm"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {pedidosFiltrados.map((pedido) => (
          <div
            key={pedido.id}
            className="bg-[#FBFBFB] border border-[#E7D1FF] rounded-xl p-6 shadow-md hover:shadow-lg transition-all"
          >
            ...
            <p className={`inline-block mt-3 px-3 py-1 rounded-full text-sm font-semibold shadow ${getBadgeColor(pedido.estado)}`}>
              {pedido.estado}
            </p>
            <div className="flex justify-end gap-3 mt-4">
              <button
                className="text-[#4D4DFF] hover:text-[#2D2DFF]"
        ...
        {pedidosFiltrados.length === 0 && (
          <p className="text-center col-span-full text-gray-500">No hay pedidos disponibles.</p>
        )}
      </div>
